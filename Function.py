...
def tagging(num, df_list, str_list, tag_list, j, df, errlog):
        ...
        try:
            ...
            if find[0] > start:
                df['문장'][j] = df['문장'].str.slice_replace(find[0], find[1], "<" + tag_list[num][1] + ">" + str_list[num+1][0] + "$" + str_list[num+1][1:] + "</" + tag_list[num][1] + ">")[j]
                str_list[0] = df['문장'][j]
            else:
                df['문장'][j] = df['문장'].str.slice_replace(find[0], find[1], "<" + tag_list[num][0] + ">" + str_list[num+1][0] + "$" + str_list[num+1][1:] + "</" + tag_list[num][0] + ">")[j]
                str_list[0] = df['문장'][j]
            ...
            while df['문장'][check] is pd.NA:
                ...
                if next_find[0] > start:
                    df['문장'][j] = df['문장'].str.slice_replace(next_find[0], next_find[1], "<" + tag_list[num][1] + ">" + str_list[num+1][0] + "$" + str_list[num+1][1:] + "</" + tag_list[num][1] + ">")[j]
                    str_list[0] = df['문장'][j]
                else :
                    df['문장'][j] = df['문장'].str.slice_replace(next_find[0], next_find[1], "<" + tag_list[num][0] + ">" + str_list[num+1][0] + "$" + str_list[num+1][1:] + "</" + tag_list[num][0] + ">")[j]
                    str_list[0] = df['문장'][j]
